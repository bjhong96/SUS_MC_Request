...
./request_fragment_check.py --bypass_status --prepid SUS-RunIISummer15wmLHEGS-00053
GEN_ERR=$?
...
# It is replaced with the one modified to match UL
#curl -s -k https://cms-pdmv-prod.web.cern.ch/mcm/public/restapi/requests/get_fragment/SUS-RunIISummer15wmLHEGS-00053 --retry 3 --create-dirs -o Configuration/GenProduction/python/SUS-RunIISummer15wmLHEGS-00053-fragment.py
#[ -s Configuration/GenProduction/python/SUS-RunIISummer15wmLHEGS-00053-fragment.py ] || exit $?;

# Check if fragment contais gridpack path ant that it is in cvmfs
if grep -q "gridpacks" /eos/user/b/byhong/MC_request/fragment_mglu1000.py; then
  if ! grep -q "/cvmfs/cms.cern.ch/phys_generator/gridpacks" Configuration/GenProduction/python/SUS-RunIISummer15wmLHEGS-00053-fragment.py; then
    echo "Gridpack inside fragment is not in cvmfs."
    exit -1
  fi
fi
# Dump actual test code to a SUS-RunIISummer15wmLHEGS-00053_test.sh file that can be run in Singularity
cat <<'EndOfTestFile' > SUS-RunIISummer15wmLHEGS-00053_test.sh
#!/bin/bash
...
chmod +x SUS-RunIISummer15wmLHEGS-00053_test.sh
...
singularity run -B /afs -B /eos -B /cvmfs -B /etc/grid-security -B /etc/pki/ca-trust --home $PWD:$PWD /cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/$CONTAINER_NAME $(echo $(pwd)/SUS-RunIISummer15wmLHEGS-00053_test.sh)
